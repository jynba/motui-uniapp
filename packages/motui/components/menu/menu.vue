...
export default defineComponent({
  ...
  setup(props) {
    const barId = `mot-menu__bar${getRandomId()}`
    const offset = ref(0)
...
<template>
  <view :class="classes" :style="customStyle">
    <view :id="barId" class="mot-menu__bar" :class="{ opened }">
      <template v-for="(item, index) in children" :key="index">
        <view
          class="mot-menu__item"
        ...
        >
          <view class="mot-menu__title" :class="getClasses(item.state.showPopup)">
            <view class="mot-menu__title-text">
              {{ item.renderTitle() }}
            </view>
            <view class="mot-menu__title-icon">
              <!-- #ifdef MP-WEIXIN -->
