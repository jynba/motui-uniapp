...
import MotTabs from '../tabs/tabs.vue'
import MotTabPane from '../tabpane/tabpane.vue'
...
<template>
  <MotTabs v-model="tabsCursor" :custom-class="classes" :custom-style="customStyle" :type="props.titleType" :ellipsis="props.titleEllipsis" :title-gutter="props.titleGutter" :size="props.titleSize" title-scroll @click="methods.handleTabClick">
    <template v-if="!initLoading && panes.length">
      <MotTabPane v-for="(pane, index) in panes" :key="index" :title="formatTabTitle(pane)">
        <view role="menu" class="mot-cascader-pane">
          <scroll-view scroll-y style="height: 100%">
            <template v-for="node in pane.nodes" :key="node.value">
              <view
                class="mot-cascader-item"
              ...
              >
                <view class="mot-cascader-item__title">
                  {{ node.text }}
                </view>

                <MotIcon v-if="node.loading" loading custom-class="mot-cascader-item__icon-loading" name="loading" />
                <MotIcon v-else custom-class="mot-cascader-item__icon-check" name="checklist" />
              </view>
            </template>
          </scroll-view>
        </view>
    ...
    <template v-else>
      <MotTabPane title="Loading...">
        <view class="mot-cascader-pane" />
      </MotTabPane>
    </template>
  </MotTabs>
</template>
