...
function findTabsIndex(value: string | number) {
  ...
  //   console.warn('[MotUI] <Tabs> 当前未找到 TabPane 组件元素 , 请检查 .')

  // else if (index === -1)
  //   console.warn('[MotUI] <Tabs> 请检查 v-model 值是否为 paneKey ,如 paneKey 未设置，请采用下标控制 .')
...
function scrollIntoView() {
  ...
  raf(() => {
    Promise.all([
      getSelectorNodeInfo(`#mot-tabs__titles_${refRandomId}`),
      getSelectorNodeInfos(`#mot-tabs__titles_${refRandomId} .mot-tabs__titles-item`),
...
<template>
  <view ref="container" :style="customStyle" :class="classes">
    <scroll-view
      :id="`mot-tabs__titles_${refRandomId}`"
      :scroll-x="getScrollX"
      :scroll-y="getScrollY"
      ...
      :enable-flex="true"
      class="mot-tabs__titles"
      :class="{ [type]: type, scrollable: titleScroll, [size]: size }"
      :style="tabsNavStyle"
    >
      <view class="mot-tabs__list" :class="{ 'mot-tabs__titles-left': align === 'left' }">
        <slot v-if="$slots.titles" name="titles" />
        <template v-else>
          <view
            v-for="(item, index) in titles"
            :key="item.paneKey"
            class="mot-tabs__titles-item uni"
            :style="titleStyle"
            :class="{
              'mot-tabs-active': item.paneKey === String(modelValue),
              'disabled': item.disabled,
              'mot-tabs__titles-item-left': align === 'left',
            }"
            @click="tabChange(item, index)"
          >
            <view v-if="type === 'line'" class="mot-tabs__titles-item__line" :style="tabsActiveStyle" />
            <view v-if="type === 'smile'" class="mot-tabs__titles-item__smile" :style="tabsActiveStyle">
              <MotIcon name="joy-smile" :custom-color="customColor" />
            </view>
            <view class="mot-tabs__titles-item__text" :class="{ ellipsis }">
              {{ item.title }}
            </view>
          </view>
          <view v-if="canShowLabel && titleScroll" class="mot-tabs__titles-placeholder" />
        </template>
      </view>
    </scroll-view>
    <view
