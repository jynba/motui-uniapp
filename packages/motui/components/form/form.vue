...
import MotCellGroup from '../cellgroup/cellgroup.vue'
...
function findFormItem(vnodes: VNode[]) {
  ...
  vnodes.forEach((vnode: VNode) => {
    ...
    if (type === 'mot-form-item' || type?.toString().endsWith('form-item')) {
      task.push({
...
async function checkRule(item: FormRule): Promise<ErrorMessage | boolean> {
  ...
  if (!prop)
    console.warn('[MotUI] <FormItem> 使用 rules 校验规则时 , 必须设置 prop 参数')
