...
<template>
  <view :class="classes" :style="customStyle">
    <view
      class="mot-input-number__icon mot-input-number__left"
      :class="{ 'mot-input-number__icon--disabled': !reduceAllow() }" @click="(reduce as any)"
    >
      <slot name="leftIcon">
        <MotIcon name="minus" :size="pxCheck(buttonSize)" />
      </slot>
    </view>
    <view v-if="props.readonly" class="mot-input-number__text--readonly">
      {{ modelValue }}
    </view>
    <template v-else>
      <!-- #ifdef MP -->
      <input
        class="mot-input-number__text--input" type="number" :min="min" :max="max"
    ...
    <view
      class="mot-input-number__icon mot-input-number__right"
      :class="{ 'mot-input-number__icon--disabled': !addAllow() }" @click="(add as any)"
    >
      <slot name="rightIcon">
        <MotIcon name="plus" :size="pxCheck(buttonSize)" />
      </slot>
    </view>
  </view>
</template>
