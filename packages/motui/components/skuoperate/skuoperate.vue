...
<template>
  <view v-if="btnOptions.length > 0" class="mot-sku-operate">
    <view v-if="btnExtraText" class="mot-sku-operate-desc">
      {{ btnExtraText }}
    </view>

    <slot name="operateBtn" />

    <view v-if="showDefaultOperate" class="mot-sku-operate-btn">
      <view
        v-for="(btn, i) in btnOptions" :key="i" class="mot-sku-operate-btn-item" :class="[`mot-sku-operate-btn-${btn}`]"
