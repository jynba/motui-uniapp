...
import MotElevator from '../elevator/elevator.vue'
...
function transformData(data: AddressRegionData[]) {
  ...
  data.forEach((item: AddressRegionData) => {
    if (!item.title)
      console.warn('[MotUI] <Address> 请检查数组选项的 title 值是否有设置 ,title 为必填项 .')
  })
...
<template>
  ...
  >
    <view :class="classes" :style="customStyle">
      <view class="mot-address__header">
        <view class="mot-address__header-back" @click="switchModule">
          <slot v-if="type === 'exist' && privateType === 'custom'" name="backIcon">
            <MotIcon name="left" size="14px" />
          </slot>
        </view>

        <view class="mot-address__header__title">
          {{
            privateType === 'custom'
        ...
        <view class="mot-address__header-close" @click="handClose('cross')">
          <slot name="closeIcon">
            <MotIcon name="close" custom-color="#cccccc" size="14px" />
          </slot>
        </view>
      </view>

      <!-- 请选择 -->
      <view v-if="['custom', 'custom2'].includes(privateType)" class="mot-address__custom">
        <view class="mot-address__region">
          <view
            v-for="(item, index) in selectedRegion" :key="index"
            class="mot-address__region-item "
            :class="[index === tabIndex ? 'active' : '']"
            @click="changeRegionTab(item, index)"
          >
            <view>{{ getTabName(item, index) }} </view>
            <view class="mot-address__region-line--mini" :class="{ active: index === tabIndex }" />
          </view>
          <view v-if="tabIndex === selectedRegion.length" class="active mot-address__region-item">
            <view>{{ getTabName(null, selectedRegion.length) }} </view>
            <view class="mot-address__region-line--mini active" />
          </view>
        </view>

        <view v-if="privateType === 'custom'" class="mot-address__detail">
          <div class="mot-address__detail-list">
            <scroll-view :scroll-y="true" :style="{ height: '100%' }" :scroll-top="scrollTop" @scroll="scrollChange">
              <div
                v-for="(item, index) in regionList"
                :key="index"
                class="mot-address__detail-item" :class="[selectedRegion[tabIndex]?.id === item.id ? 'active' : '']"
                @click="nextAreaList(item)"
              >
                <view>
                  <slot v-if="selectedRegion[tabIndex]?.id === item.id" name="icon">
                    <MotIcon name="Check" custom-class="mot-address-select-icon" width="13px" />
                  </slot>{{ item.name }}
                </view>
              </div>
            </scroll-view>
        ...
        <view v-else class="mot-address__elevator-group">
          <MotElevator
            :height="height"
      ...
      <!-- 配送至 -->
      <view v-else-if="privateType === 'exist'" class="mot-address__exist">
        <div class="mot-address__exist-group">
          <ul class="mot-address__exist-group-list">
            <li
              v-for="(item, index) in existAddress"
              :key="index"
              class="mot-address__exist-group-item"
              :class="[item.selectedAddress ? 'active' : '']"
              @click="selectedExist(item)"
            >
              <slot v-if="!item.selectedAddress" name="unselectedIcon">
                <MotIcon name="location2" custom-class="mot-address-select-icon" width="13px" />
              </slot>
              ...
              <div class="mot-address__exist-item-info">
                <div v-if="item.name && item.phone" class="mot-address__exist-item-info-top">
                  <div class="mot-address__exist-item-info-name">
                    {{ item.name }}
                  </div>
                  <div class="mot-address__exist-item-info-phone">
                    {{ item.phone }}
                  </div>
                </div>

                <div class="mot-address__exist-item-info-bottom">
                  <view>
        ...
        <div v-if="isShowCustomAddress" class="mot-address__exist-choose" @click="switchModule">
          <div class="mot-address__exist-choose-btn">
            {{
