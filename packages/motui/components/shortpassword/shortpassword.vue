...
<template>
  <view :class="classes" :style="customStyle">
    <MotPopUp
      ...
      @click-overlay="close"
    >
      <view class="mot-short-password-title">
        {{ title || translate('title') }}
      </view>
      <view class="mot-short-password-subtitle">
        {{ desc || translate('desc') }}
      </view>
      <view class="mot-short-password-wrapper">
        <view class="mot-short-password__list" @touchstart="focus">
          <view v-for="(sublen, index) in new Array(comLen)" :key="index" class="mot-short-password__item">
            <view v-if="String(realInput).length > index" class="mot-short-password__item-icon" />
          </view>
        </view>
      </view>
      <view class="mot-short-password__message">
        <view class="mot-short-password--error">
          {{ errorMsg }}
        </view>
        <view v-if="tips || translate('tips')" class="mot-short-password--forget" @click="onTips">
          <MotIcon name="tips" custom-class="icon" size="11px" />
