...
import MotSwiper from '../swiper/swiper.vue'
import MotSwiperItem from '../swiperitem/swiperitem.vue'
...
<template>
  <MotPopup v-model:visible="state.showPop" :lock-scroll="true" pop-class="mot-image-preview-custom-pop">
    <view :class="classes" :style="customStyle" @touchstart.capture="(onTouchStart as any)">
      <MotSwiper
        v-if="state.showPop"
        :auto-play="autoplay"
        custom-class="mot-image-preview-swiper"
        :loop="isLoop"
        ...
        @change="setActive"
      >
        <MotSwiperItem v-for="(item, index) in images" :key="index">
          <movable-area class="mot-image-movable-area">
            <movable-view :disabled="!scale" inertia scale-area class="mot-image-preview-img" :scale="scale" direction="all">
              <image
                mode="aspectFit"
                :src="item.src"
                class="mot-image-preview-img"
    ...
    <view v-if="showIndex" class="mot-image-preview-index">
      {{ state.active + 1 }} / {{ images.length }}
    </view>
    <view v-if="closeable" class="mot-image-preview-close-icon" :style="styles" @click="onClose">
      <MotIcon name="circle-close" custom-color="#ffffff" />
    </view>
  </MotPopup>
</template>
