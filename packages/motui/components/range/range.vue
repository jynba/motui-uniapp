...
const containerClasses = computed(() => {
  const prefixCls = 'mot-range-container'
  return {
    [prefixCls]: true,
    [`${prefixCls}-vertical`]: props.vertical,
  }
...
function markClassName(mark: number) {
  const classPrefix = 'mot-range-mark'
...
<template>
  <view :class="[containerClasses, customClass]" :style="customStyle">
      ...
      <view class="mot-range-bar" :style="barStyle">
        <template v-if="range">
          ...
          >
            ...
            <view v-else class="mot-range-button" :style="buttonStyle">
        ...
        <template v-else>
          <view
            role="slider" class="mot-range-button-wrapper" :tabindex="disabled ? -1 : 0" :aria-valuemin="+min"
    ...
    <view v-if="!hiddenRange" class="mot-range-max">
      {{ +max }}
    </view>
  </view>
</template>
