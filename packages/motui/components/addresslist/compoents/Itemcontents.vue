...
<template>
  <view class="mot-address-list-item" @click="handleContentsClick">
    <view class="mot-address-list-item__info">
      <view class="mot-address-list-item__info-contact">
        <slot v-if="props.useContentTopSlot" name="content-top" />

        <template v-else>
          <view class="mot-address-list-item__info-contact-name">
            {{ props.item.addressName }}
          </view>
          <view class="mot-address-list-item__info-contact-tel">
            {{ props.item.phone }}
          </view>
          <view v-if="props.item.defaultAddress" class="mot-address-list-item__info-contact-default">
            {{ translate('default') }}
          </view>
        </template>
      </view>

      <view class="mot-address-list-item__info-handle">
        <slot v-if="props.useContentIconSlot" name="content-icon" />

        <template v-else>
          <MotIcon name="del" custom-class="mot-address-list-item__info-handle-del" @tap.stop="handleDelIconClick" />
          <MotIcon name="edit" custom-class="mot-address-list-item__info-handle-edit" @tap.stop="handleEditIconClick" />
        </template>
      </view>
    </view>

    <view class="mot-address-list-item__addr">
