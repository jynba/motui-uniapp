...
<template>
  <view :class="classes" :style="customStyle">
    <view
      class="mot-pagination-prev" :class="[mode === 'multi' ? '' : 'simple-border', modelValue === 1 ? 'disabled' : '']"
    ...
    <view v-if="mode === 'multi'" class="mot-pagination-contain">
      <view
        v-for="(item, index) of pages"
        :key="`${index}pagination`"
        class="mot-pagination-item" :class="[item.active ? 'active' : '']"
    ...
    <view v-if="mode === 'simple'" class="mot-pagination-contain">
      <view class="mot-pagination-simple">
        {{ modelValue }}/{{ countRef }}
      </view>
    </view>
    <view
      class="mot-pagination-next" :class="[modelValue >= countRef ? 'disabled' : '']"
      @click="select(modelValue + 1, true)"
    >
      <slot name="nextText">
