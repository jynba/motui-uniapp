...
const wrapContentClass = computed(() => {
  return {
    'mot-noticebar__page-wrap-content': true,
    'mot-ellipsis': isEllipsis.value,
...
<template>
  <view :class="classes" :style="customStyle">
    <view
      ...
      class="mot-noticebar__page"
      :class="{
        'mot-noticebar__page--withicon': closeMode,
        'mot-noticebar__page--close': closeMode,
        'mot-noticebar__page--wrapable': wrapable,
        'mot-hidden': !state.showNoticebar,
      }"
      :style="barStyle"
      @click="(handleClick as any)"
    >
      <view v-if="leftIcon" class="mot-noticebar__page-lefticon">
        <slot name="leftIcon">
          <MotIcon name="notice" size="16px" />
        </slot>
      </view>
      <view ref="wrap" :class="`mot-noticebar__page-wrap wrap${state.id}`">
        <view
          ref="content"
      ...
      <view v-if="closeMode || $slots.rightIcon" class="mot-noticebar__page-righticon" @click.stop="(onClickIcon as any)">
        <slot name="rightIcon">
          <MotIcon name="circle-close" />
        </slot>
      </view>
    </view>
    ...
    <view
      ...
      class="mot-noticebar__vertical"
      :style="barStyle"
    >
      <view class="mot-noticebar__vertical-list" :style="horseLampStyle">
        <view
          v-for="(item, index) in state.scrollList"
          :key="index"
          class="mot-noticebar__vertical-item"
      ...
      <view class="go" @click="handleClickIcon()">
        <slot name="rightIcon">
          <MotIcon v-if="closeMode" name="circle-close" :custom-color="customColor" size="11px" />
        </slot>
      </view>
    </view>
  </view>
