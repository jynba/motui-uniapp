...
<template>
  <MotPopup
    ...
    :pop-class="popClass"
    :overlay="overlay"
    overlay-class="mot-number-keyboard-overlay"
    @click-overlay="closeBoard()"
  >
    <div ref="root" :class="classes" :style="customStyle">
      <div v-if="title" class="mot-number-keyboard__header">
        <h3 class="mot-number-keyboard__title">
          {{ title }}
        </h3>
        <text v-if="type === 'default'" class="mot-number-keyboard__close" @click="closeBoard()">
          {{
            translate('done')
          }}
        </text>
      </div>
      <div class="mot-number-keyboard__body">
        <div class="mot-number-keyboard__keys">
          <div
            v-for="item of keysList" :key="`key${item.id}`"
            class="mot-key__wrapper"
            :class="[
              {
                'mot-key__wrapper--wider':
          ...
          >
            <div
              class="mot-key" :class="[
                { 'mot-key--active': item.id === clickKeyIndex },
                { 'mot-key--lock': item.type === 'lock' },
                { 'mot-key--delete': item.type === 'delete' },
              ]"
        ...
        <div v-if="type === 'rightColumn'" class="mot-number-keyboard__sidebar">
          <div class="mot-key__wrapper">
            <div
              class="mot-key" :class="[{ active: clickKeyIndex === 'delete' }]"
          ...
          <div class="mot-key__wrapper mot-key__wrapper--finish" @click="onConfirm">
            <div class="mot-key mot-key--finish " :class="[{ activefinsh: clickKeyIndex === 'finish' }]">
