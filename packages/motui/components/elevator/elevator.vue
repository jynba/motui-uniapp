...
<template>
  <view :class="classes" :style="customStyle">
    <scroll-view
      id="listview"
      class="mot-elevator__list mot-elevator__list--mini"
    ...
    >
      <view id="elevator__item-wrap">
        <view
          ...
          :key="item[acceptKey]"
          class="mot-elevator__list__item"
        >
          <view class="mot-elevator__list__item__code">
            {{ item[acceptKey] }}
          </view>
          <view
            v-for="subitem in item.list"
            :key="subitem.id"
            class="mot-elevator__list__item__name"
            :class="{
              'mot-elevator__list__item__name--highcolor': currentData.id === subitem.id && currentKey === item[acceptKey],
            }"
    ...
    <view class="mot-elevator__list__fixed" :class="{ 'mot-hidden': !(scrollY > 2 && isSticky) }">
      <view class="mot-elevator__list__fixed-title">
        {{ indexList[currentIndex][acceptKey] }}
      </view>
    </view>
    <view v-if="indexList.length > 0" class="mot-elevator__code--current" :class="{ 'mot-hidden': !scrollStart }">
      {{ indexList[codeIndex][acceptKey] }}
    </view>
    <view class="mot-elevator__bars" @touchstart="(touchStart as any)" @touchmove.stop.prevent="(touchMove as any)" @touchend="touchEnd">
      <view class="mot-elevator__bars__inner">
        <view
          v-for="(item, index) in indexList"
          :key="item[acceptKey]"
          class="mot-elevator__bars__inner__item"
