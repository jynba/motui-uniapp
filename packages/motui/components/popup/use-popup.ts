...
export function usePopup(props: PopupProps, emit: SetupContext<PopupEmits>['emit']) {
  ...
  const classes = computed(() => {
    return getMainClass(props, componentName, {
      round: props.round,
      [`mot-popup--${props.position}`]: true,
      [`mot-popup--${props.position}--safebottom`]: props.position === 'bottom' && props.safeAreaInsetBottom,
      [`mot-popup--${props.position}--safetop`]: props.position === 'top' && props.safeAreaInsetTop,
      [props.popClass]: true,
    })
  })
