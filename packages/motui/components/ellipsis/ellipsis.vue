...
<template>
  <view :class="customClass" :style="customStyle">
    <view :id="rootId" :class="classes" @click="handleClick">
      <view v-if="!state.exceeded" class="mot-ellipsis__wordbreak">
        {{ content }}
      </view>

      <view v-if="state.exceeded && !state.expanded" class="mot-ellipsis__wordbreak">
        {{ ellipsis.leading }}{{ ellipsis.leading && symbol
        }}
        <view v-if="expandText" class="mot-ellipsis__text" @click.stop="clickHandle(1)">
          {{ expandText }}
        </view>{{ ellipsis.tailing && symbol }}{{ ellipsis.tailing }}
      </view>
      <view v-if="state.exceeded && state.expanded">
        {{ content }}
        <view v-if="expandText" class="mot-ellipsis__text" @click.stop="clickHandle(2)">
          {{ collapseText }}
        </view>
      </view>
    </view>

    <view :id="rootContainId" class="mot-ellipsis__copy" :style="{ width: widthRef }">
      <view>{{ contantCopy }}</view>
    </view>

    <!-- 省略号 symbol  -->
    <view :id="symbolContainId" class="mot-ellipsis__copy" style="display: inline">
      {{
        symbolText
      }}
    </view>
