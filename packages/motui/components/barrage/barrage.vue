...
const classes = computed(() => {
  return getMainClass(props, componentName, {
    [`mot-barrage--dmBody${timeId.value}`]: true,
  })
})
...
onMounted(() => {
  ...
  if (slotDefault) {
    const list = document
      .getElementsByClassName(`mot-barrage__slotBody${classTime}`)[0]
      .getElementsByClassName('mot-barrage__item')
...
function getNode(index: number) {
  setTimeout(async () => {
    let width = 100
    const dmBodyNodeInfo = await getSelectorNodeInfo(`.mot-barrage--dmBody${timeId.value}`)
    width = dmBodyNodeInfo?.width || 300
    const itemNodeInfo = await getSelectorNodeInfo(`.mot-barrage__item${index}`)
...
<template>
  <view :class="classes" :style="customStyle">
    <div>
      <div :class="[`mot-barrage__slotBody${classTime}`]">
        <view
          v-for="(item, index) of danmuList"
          :key="`danmu${index}`"
          class="mot-barrage__item move" :class="[`mot-barrage__item${index}`]"
