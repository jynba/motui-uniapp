...
import MotProgress from '../progress/progress.vue'
...
<template>
  <view :class="classes" :style="customStyle">
    <view v-if="$slots.default" class="mot-uploader__slot">
      <slot />
      <template v-if="Number(maximum) - fileList.length">
        <MotButton custom-class="mot-uploader__input" @click="(chooseImage as any)" />
      </template>
    </view>

    <view v-for="(item, index) in fileList" :key="item.uid" class="mot-uploader__preview" :class="[listType]">
      <view v-if="listType === 'picture' && !$slots.default" class="mot-uploader__preview-img">
        <view v-if="item.status !== 'success'" class="mot-uploader__preview__progress">
          <template v-if="item.status !== 'ready'">
            <MotIcon v-if="item.status === 'error'" name="failure" custom-color="#fff" />
            <MotIcon v-else name="loading" custom-color="#fff" />
          </template>
          <view class="mot-uploader__preview__progress__msg">
            {{ item.message }}
          </view>
        </view>
        ...
        <view v-if="isDeletable && !disabled" class="close" @click="onDelete(item, index)">
          <slot name="deleteIcon">
            <MotIcon name="failure" />
          </slot>
        </view>

        <image
          v-if="(item?.type?.includes('image') || item?.type?.includes('video')) && item.url"
          class="mot-uploader__preview-img__c"
          :mode="mode"
          :src="item.url"
          @click="fileItemClick(item)"
        />
        <view v-else class="mot-uploader__preview-img__file">
          <view class="mot-uploader__preview-img__file__name" @click="fileItemClick(item)">
      ...
      <view v-else-if="listType === 'list'" class="mot-uploader__preview-list">
        <view class="mot-uploader__preview-img__file__name" :class="[item.status]" @click="fileItemClick(item)">
          <MotIcon name="link" custom-class="mot-uploader__preview-img__file__link" />
          <view class="file-name__tips">
            {{ item.name }}
          </view>
          <MotIcon
            ...
            name="del"
            custom-color="#808080"
            custom-class="mot-uploader__preview-img__file__del"
    ...
    <view
      ...
      class="mot-uploader__upload"
      :class="[listType]"
    >
      <slot name="uploadIcon">
        <MotIcon name="photograph" custom-color="#808080" />
      </slot>
      <MotButton custom-class="mot-uploader__input" :class="{ disabled }" @click="(chooseImage as any)" />
    </view>
  </view>
</template>
