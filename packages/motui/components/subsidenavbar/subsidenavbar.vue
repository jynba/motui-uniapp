...
<template>
  <view :class="classes" :style="customStyle" :ikey="ikey">
    <view class="mot-sub-side-navbar__title" :style="[paddingStyle]" @click.stop="handleClick">
      <text class="mot-sub-side-navbar__title__text">
        {{ title }}
      </text>
      <view class="mot-sub-side-navbar__title__icon">
        <MotIcon v-if="!direction" custom-class="icon" name="arrow-down2" size="12px" />
        <MotIcon v-else name="arrow-up2" size="12px" />
      </view>
    </view>
    <view
      class="mot-sub-side-navbar__list"
      :class="[{ 'mot-hidden': direction }, direction ? 'motFadeOut' : 'motFadeIn']"
      :style="[style]"
    >
      <slot />
    </view>
