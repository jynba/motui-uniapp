...
const classes = computed(() => {
  return getMainClass(props, componentName, {
    'mot-calendar--nopop': !props.poppable,
    'mot-calendar--nofooter': props.isAutoBackFill,
  })
})
// 新增：自定义周起始日
...
function isActive(day: Day, month: MonthInfo) {
  return (
    ...
    && day.type === 'curr'
    && getClass(day, month).includes('mot-calendar__day--active')
  )
}
...
<template>
  ...
  >
    <!-- header -->
    <view class="mot-calendar__header">
      <view v-if="showTitle" class="mot-calendar__header-title">
        {{ title || translate('title') }}
      </view>
      <view v-if="btnSlot" class="mot-calendar__header-slot">
        <slot name="btn" />
      </view>
      <view v-if="showSubTitle" class="mot-calendar__header-subtitle">
        {{ state.yearMonthTitle }}
      </view>
      <view class="mot-calendar__weekdays">
        <view
          v-for="(item, index) of weeks"
          :key="index"
          class="mot-calendar__weekday"
    ...
    <scroll-view
      ...
      :scroll-y="true"
      class="mot-calendar__content"
      :scroll-with-animation="scrollWithAnimation"
      @scroll="mothsViewScroll"
    >
      <view class="mot-calendar__panel" :style="{ height: state.containerHeight }">
        <view class="mot-calendar__body" :style="{ transform: `translateY(${state.translateY}px)` }">
          <view v-for="(month, index) of compConthsData" :key="index" class="mot-calendar__month">
            <view class="mot-calendar__month-title">
              {{ month.title }}
            </view>
            <view class="mot-calendar__days">
              <view class="mot-calendar__days-item" :class="type === 'range' ? 'mot-calendar__days-item--range' : ''">
                <template v-for="(day, i) of month.monthData" :key="i">
                  <view class="mot-calendar__day" :class="getClass(day, month, i)" @click="chooseDay(day, month)">
                    <!-- 日期显示slot -->
                    <view class="mot-calendar__day-value">
                      <!-- #ifdef MP -->
                    ...
                    <!-- #ifdef H5 -->
                    <view v-if="topInfo" class="mot-calendar__day-tips mot-calendar__day-tips--top">
                      <slot name="topInfo" :date="day.type === 'curr' ? day : ''" />
                    </view>
                    <view v-if="bottomInfo" class="mot-calendar__day-tips mot-calendar__day-tips--bottom">
                      <slot name="bottomInfo" :date="day.type === 'curr' ? day : ''" />
                    </view>
                    <!-- #endif -->
                    <!-- #ifndef MP -->
                    <view v-if="!bottomInfo && showToday && isCurrDay(day)" class="mot-calendar__day-tips--curr">
                      {{ translate('today') }}
                    </view>
                    <!-- #endif -->
                    <!-- #ifdef MP -->
                    <view v-if="showToday && isCurrDay(day)" class="mot-calendar__day-tips--curr">
                      {{ translate('today') }}
                    </view>
                    <!-- #endif -->
                    <view
                      v-if="isStartTip(day, month)"
                      class="mot-calendar__day-tip" :class="{ 'mot-calendar__day-tips--top': rangeTip() }"
                    >
                      {{ startText || translate('start') }}
                    </view>
                    <view v-if="isEndTip(day, month)" class="mot-calendar__day-tip">
                      {{
    ...
    <!-- footer -->
    <view v-if="poppable && !isAutoBackFill" class="mot-calendar__footer">
      <slot v-if="footerSlot" name="footer" :date="state.chooseData" />

      <view v-else class="mot-calendar__confirm" @click="confirm">
