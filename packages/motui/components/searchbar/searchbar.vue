...
<template>
  <view :class="classes" :style="styles">
    <view
      ...
      class="mot-searchbar__search-icon mot-searchbar__left-search-icon"
      @click="handleLeftIconClick"
    >
      <slot name="leftout" />
    </view>
    <view class="mot-searchbar__search-input" :class="[props.shape]" :style="inputWrapperStyles">
      <view v-if="hasSlot('leftin')" class="mot-searchbar__search-icon mot-searchbar__iptleft-search-icon">
        <slot name="leftin" />
      </view>
      <view class="mot-searchbar__input-inner" :class="{ 'mot-searchbar__input-inner-absolute': hasSlot('rightin') }">
        <form class="mot-searchbar__input-form" action="#" onsubmit="return false" @submit.prevent="handleSubmit">
          <input
            class="mot-searchbar__input-bar"
            :class="{ 'mot-searchbar__input-bar_clear': props.clearable }"
            :style="inputStyles"
      ...
      <view
        class="mot-searchbar__input-inner-icon"
        :class="{ 'mot-searchbar__input-inner-icon-absolute': hasSlot('rightin') }"
      >
        <view
          v-if="props.clearable"
          class="mot-searchbar__search-icon mot-searchbar__input-clear"
          :class="{ 'mot-hidden': innerValue.length <= 0 }"
          @click="handleClear"
        >
          <template v-if="hasSlot('clear-icon')">
            <slot name="clear-icon" />
          </template>
          <MotIcon v-else :name="props.clearIcon" />
        </view>
        <view
          v-if="hasSlot('rightin')"
          class="mot-searchbar__search-icon mot-searchbar__iptright-search-icon"
    ...
    <view v-if="hasSlot('rightout')" class="mot-searchbar__search-icon mot-searchbar__right-search-icon">
      <slot name="rightout" />
    </view>
  </view>
</template>
