...
function countGo(flag?: number): void {
  ...
  // 数字都是从小加到大的，所以我们循环转动最后一个数字，传入最后一个数字的DOM
    nextTick(() => {
      ...
      const refsDom: HTMLCollectionOf<Element> = document.getElementsByClassName('mot-countup__number-item')
...
function imgNumberScroll() {
  ...
  // #ifdef H5
  nextTick(() => {
    const f = document.getElementsByClassName('mot-countup__numberimg')[0]
    // setTimeout(() => {
  ...
  <view :class="classes" :style="customStyle">
    <template v-if="customBgImg !== ''">
      <template v-if="type === 'machine'">
        <view class="mot-countup__machine" :style="{ height: `${numHeight}px` }">
          <view
            v-for="(val, index) of machineNum"
            :key="`mImg${index}`"
            class="mot-countup__machine-item"
            :style="{
              width: `${numWidth}px`,
              height: `${numHeight}px`,
      ...
      <template v-else>
        <view class="mot-countup__numberimg" :style="{ height: `${numHeight}px` }">
          <view
            v-for="(val, index) of data.num_total_len"
            :key="`cImg${index}`"
            class="mot-countup__numberimg__item"
            :style="{
              width: `${numWidth}px`,
              height: `${numHeight}px`,
              left:
    ...
    <template v-else>
      ...
      >
        <view
          v-for="(val, index) of data.num_total_len"
          :key="val"
          class="mot-countup__number-item"
          :style="{
        ...
        >
          <view
            ...
            :key="`dote${idx}`"
            class="mot-countup__number-item__span"
            :style="{
              width: `${numWidth}px`,
              height: `${numHeight}px`,
              lineHeight: `${numHeight}px`,
