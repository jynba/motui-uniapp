...
<template>
  <view class="mot-navbar--placeholder" :style="{ height: navHeight }">
    <view id="navBarHtml" :class="classes" :style="styles">
      <view class="mot-navbar__left" @click="handleLeft">
        <slot v-if="leftShow" name="leftShow">
          <MotIcon custom-class="right-icon" name="left" height="12px" :size="size" :custom-color="customColor" @click="handleBack" />
        </slot>
        <view v-if="leftText" :style="colorStyle" class="mot-navbar__text">
          {{ leftText }}
        </view>
        <slot name="left" />
      </view>
      <view class="mot-navbar__title">
      ...
      <view class="mot-navbar__right" @click="handleRight">
        <view v-if="desc" :style="customStyle" class="mot-navbar__text">
          {{ desc }}
        </view>
        <slot name="right" />
      </view>
