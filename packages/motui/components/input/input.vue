...
<template>
  <view :class="classes" :style="props.customStyle" @click="handleClick">
    <view v-if="hasSlot('left')" class="mot-input__left">
      <slot name="left" />
    </view>

    <view class="mot-input__value">
      <input
        class="mot-input__input"
      ...
      <view v-if="props.readonly" class="mot-input__mask" @click="handleClickInput" />

      <view v-if="props.showWordLimit && innerMaxLength > 0" class="mot-input__word-limit">
        <text class="mot-input__word-num">
          {{ innerValue.length }}
        </text>/{{ innerMaxLength }}
      </view>
    </view>
    ...
    <view
      ...
      class="mot-input__clear"
      :class="{ 'mot-hidden': !((active || props.showClearIcon) && innerValue.length > 0) }"
    ...
    >
      ...
      <MotIcon
        v-else
        name="mask-close"
        custom-class="mot-input__clear-icon"
    ...
    <view v-if="hasSlot('right')" class="mot-input__right">
      <slot name="right" />
    </view>
  </view>
</template>
