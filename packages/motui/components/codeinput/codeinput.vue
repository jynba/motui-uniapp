...
<template>
  <view :class="classes" :style="customStyle">
    <view v-for="(item, index) in codeLength" :key="index" class="mot-code-input__item" :style="[itemStyle(index)]">
      <view v-if="dot && codeArray.length > index" class="mot-code-input__item__dot" />
      <text
        v-else :style="{
      ...
      <view v-if="mode === 'line'" class="mot-code-input__item__line" :style="[lineStyle]" />
      <!-- #ifndef APP-PLUS -->
      <view
        v-if="isFocus && codeArray.length === index" :style="{ backgroundColor: customColor }"
        class="mot-code-input__item__cursor"
      />
      <!-- #endif -->
    </view>
    <input
      :disabled="disabledKeyboard" type="number" :focus="focus" :value="inputValue" :maxlength="+maxlength"
      :adjustPosition="adjustPosition" class="mot-code-input__input" :style="{
        height: `${props.size}px`,
