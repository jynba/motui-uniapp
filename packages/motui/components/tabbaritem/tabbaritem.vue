...
import MotBadge from '../badge/badge.vue'
...
const classes = computed(() => {
  return getMainClass(props, componentName, {
    'mot-tabbar-item__icon--unactive': !active.value,
  })
})
...
<template>
  <div
    ...
    @click="change()"
  >
    <MotBadge v-bind="props">
      <view class="mot-tabbar-item_icon-box">
        <div v-if="isHaveSlot('icon')" class="mot-tabbar-item_icon-box_icon">
          <slot name="icon" :active="active" />
        </div>
        <view v-if="icon && !isHaveSlot('icon')">
          <MotIcon :name="icon" custom-class="mot-popover-item-img" />
        </view>

        <view
          class="mot-tabbar-item_icon-box_nav-word" :class="[
            { 'mot-tabbar-item_icon-box_big-word': !icon && !isHaveSlot('icon') },
          ]"
        >
          <slot>
            <view v-if="tabTitle">
