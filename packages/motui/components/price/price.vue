...
<template>
  <view :class="classes" :style="customStyle">
    <rich-text
      ...
      class="mot-price--symbol"
      :class="`mot-price--symbol-${size}`"
      :nodes="showSymbol"
    />
    <view :class="`mot-price--${size}`">
      {{ formatThousands(price) }}
    </view>
    <view v-if="decimalDigits !== 0" :class="`mot-price--decimal-${size}`">
      .
    </view>
    <view :class="`mot-price--decimal-${size}`">
      {{ formatDecimal(price) }}
    </view>
    <rich-text
