...
<template>
  <view :class="classes" :style="styles">
    <MotOverlay v-if="overlay" :visible="visible" :z-index="200" @click="updateValue(false)" />
    <slot name="list">
      <view class="mot-fixed-nav__list">
        <view
          v-for="(item, index) in navList" :key="item.id || index"
          class="mot-fixed-nav__list-item" :class="{ active: item.id === current }" @click="selected(item, $event as any)"
        >
          <image :src="item.icon" />
          <view class="span">
            {{ item.text }}
    ...
    <div class="mot-fixed-nav__btn" @click="updateValue()">
      <slot name="btn">
        <MotIcon name="left" custom-color="#fff" />
        <view class="text">
