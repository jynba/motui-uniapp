...
<template>
  <view class="mot-sku-select">
    <view v-for="(item, index) in skuInfo" :key="item.id" class="mot-sku-select-item">
      <view class="mot-sku-select-item-title">
        {{ item.name }}
      </view>
      <view class="mot-sku-select-item-skus">
        <view
          v-for="(itemAttr, itemAttrIndex) in item.list" :key="itemAttr.name" class="mot-sku-select-item-skus-sku"
