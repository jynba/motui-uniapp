...
const classes = computed(() => {
  return getMainClass(props, componentName, {
    [`mot-popover--${props.theme}`]: true,
  })
})
const popoverArrow = computed(() => {
  const prefixCls = 'mot-popover-arrow'
...
<template>
  <view v-if="!targetId" :id="popoverID" class="mot-popover-wrapper" @click="openPopover">
    <slot name="reference" />
  </view>
  <view :class="classes" :style="getRootPosition">
    <MotPopup
      v-model:visible="showPopup"
      :destroy-on-close="false" :pop-class="`mot-popover-content mot-popover-content--${location}`"
      :custom-style="styles" :position="`` as any" :transition="`mot-popover` as any" :overlay="overlay"
      :duration="+duration" :overlay-style="overlayStyle" :overlay-class="overlayClass"
      :close-on-click-overlay="closeOnClickOverlay"
    >
      <view :id="popoverContentID" class="mot-popover-content-group" @click.stop="clickContent">
        <view v-if="showArrow" :class="popoverArrow" :style="popoverArrowStyle" />
        <slot name="content" />
        <view
          v-for="(item, index) in list" :key="index"
          class="mot-popover-menu-item" :class="[item.className, item.disabled && 'mot-popover-menu-disabled']"
          @click="chooseItem(item, index)"
        >
          <MotIcon v-if="item.icon" :name="item.icon" custom-class="mot-popover-item-img" />
          <view class="mot-popover-menu-item-name">
            {{ item.name }}
          </view>
        </view>
      </view>
    ...
    <view
      class="mot-popover-content-bg"
      :class="{ 'mot-hidden': !showPopup }"
      @touchmove="clickAway"
      @click="clickAway"
    />
  </view>
