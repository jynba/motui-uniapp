...
const classes = computed(() => {
  return getMainClass(props, componentName, {
    [isActive.value ? 'mot-switch-open' : 'mot-switch-close']: true,
...
<template>
  <view :class="classes" :style="styles" @click="(onClick as any)">
    <view class="mot-switch-button">
      <slot v-if="loading" name="icon">
        <MotIcon name="loading1" :custom-color="activeColor" />
      </slot>
      <template v-if="activeText">
        <view class="mot-switch-label open" :class="{ 'mot-hidden': !isActive }">
          {{ activeText }}
        </view>
        <view class="mot-switch-label close" :class="{ 'mot-hidden': isActive }">
          {{ inactiveText }}
        </view>
      </template>
    </view>
