...
const target = `#mot-collapse__item-${refRandomId}`
...
<template>
  <view :class="classes" :style="customStyle">
    <view
      class="mot-collapse-item__title" :class="[{ 'mot-collapse-item__title--disabled': disabled }]"
      @click="handleClick"
    >
      <view class="mot-collapse-item__title-main">
        <view class="mot-collapse-item__title-main-value">
          <slot v-if="$slots.title" name="title" />
          <template v-else>
            <rich-text class="mot-collapse-item__title-mtitle" :nodes="title" />
          </template>
          <view v-if="label" class="mot-collapse-item__title-label">
            {{ label }}
          </view>
        </view>
      </view>
      <view v-if="$slots.value" class="mot-collapse-item__title-sub">
        <slot name="value" />
      </view>
      <rich-text v-else class="mot-collapse-item__title-sub" :nodes="value" />
      <view
        class="mot-collapse-item__title-icon" :class="[{ 'mot-collapse-item__title-icon--expanded': expanded }]"
    ...
    <view v-if="$slots.extra" class="mot-collapse__item-extraWrapper">
      <div class="mot-collapse__item-extraWrapper__extraRender">
        <slot name="extra" />
      </div>
    </view>
    <view
      class="mot-collapse__item-wrapper"
      :style="{
        willChange: 'height',
        height: wrapperHeight,
      }"
    >
      <view :id="`mot-collapse__item-${refRandomId}`" class="mot-collapse__item-wrapper__content">
        <slot />
      </view>
    </view>
  </view>
