...
<template>
  <view class="mot-comment-bottom">
    <view class="mot-comment-bottom__lable" @click="handleClick">
      <span v-if="type !== 'complex'" style="display: inline;white-space:none;">
        {{ info.size }}
      </span>
    </view>

    <view class="mot-comment-bottom__cpx">
      <template v-for="(name, i) in mergeOp" :key="i">
        <view class="mot-comment-bottom__cpx-item" :class="[`mot-comment-bottom__cpx-item--${name}`]" @click="operate(name)">
          <template v-if="name !== 'more'">
            <text>{{ info[name] }}</text>
            <MotIcon v-if="name === 'like'" name="fabulous" />
            <MotIcon v-else name="comment" />
          </template>
          <template v-if="name === 'more'">
            <MotIcon name="more-x" />
            <view v-if="showPopver" class="mot-comment-bottom__cpx-item-popover" @click="operate('popover')">
              {{
