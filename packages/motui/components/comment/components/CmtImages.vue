...
<template>
  <view :class="`mot-comment-images mot-comment-images--${type}`">
    <!-- videos -->
    <view
      v-for="(itV, index) in videos" :key="itV.id" class="mot-comment-images__item mot-comment-images__item--video"
      @click="showImages('video', index)"
    >
      <image :src="itV.mainUrl" />
      <view class="mot-comment-images__play" />
    </view>
    <!-- images -->
    <template v-for="(itI, index) in images" :key="itI.id">
      <view
        v-if="(type === 'multi' && videos.length + index < 9) || type !== 'multi'"
        class="mot-comment-images__item mot-comment-images__item--imgbox"
      ...
      >
        ...
        <view
          ...
          class="mot-comment-images__mask"
        >
          <text>共 {{ totalImages.length }} 张</text>
          <MotIcon name="right" size="12px" />
        </view>
      </view>
    </template>
  </view>
