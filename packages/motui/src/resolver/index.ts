...
export function MotResolver(): ComponentResolver {
  return {
    type: 'component',
    resolve: (name: string) => {
      if (name.match(/^(Mot[A-Z]|mot-[a-z])/)) {
        const cName = name.slice(3).replace(/([a-z])/, '$1').toLowerCase()
        const component = `motui-uniapp/components/${cName}/${cName}.vue`
        const style = `motui-uniapp/components/${cName}/index.scss`
