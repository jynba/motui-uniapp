...
import type { FormItemRule, FormItemRuleWithoutValidator } from 'motui-uniapp'
...
<template>
  <div class="demo full">
    ...
    <mot-form>
      <mot-form-item label="昵称">
        <mot-input v-model="basicData.name" custom-class="mot-input-text" placeholder="请输入昵称" type="text" />
      </mot-form-item>
      <mot-form-item label="网龄">
        <mot-input v-model="basicData.age" custom-class="mot-input-text" placeholder="请输入网龄" type="text" />
      </mot-form-item>
      <mot-form-item label="时间">
        <mot-input v-model="basicData.time" custom-class="mot-input-text" placeholder="请输入时间" type="text" />
      </mot-form-item>
      <mot-form-item label="地址">
        <mot-input v-model="basicData.address" custom-class="mot-input-text" placeholder="请输入地址" type="text" />
      </mot-form-item>
    ...
    <mot-form ref="dynamicRefForm" :model-value="dynamicForm.state">
      <mot-form-item label="昵称" prop="name" required :rules="[{ required: true, message: '请填写姓名' }]">
        <mot-input v-model="dynamicForm.state.name" custom-class="mot-input-text" placeholder="请输入姓名" type="text" />
      </mot-form-item>
      <mot-form-item
      ...
      >
        <mot-input v-model="item.value" custom-class="mot-input-text" :placeholder="`请输入时间${index}`" type="text" />
      </mot-form-item>
      <mot-cell>
        <mot-button size="small" custom-style="margin-right: 10px" @click="dynamicForm.methods.add">
          添加
        </mot-button>
        <mot-button size="small" custom-style="margin-right: 10px" @click="dynamicForm.methods.remove">
          删除
        </mot-button>
        <mot-button type="primary" custom-style="margin-right: 10px" size="small" @click="dynamicForm.methods.submit">
          提交
        </mot-button>
        <mot-button size="small" @click="dynamicForm.methods.reset">
          重置提示状态
        </mot-button>
      </mot-cell>
    </mot-form>
    ...
    >
      ...
      <mot-form-item label="地址" prop="address" required :rules="[{ required: true, message: '请填写地址' }]">
        <mot-input v-model="formData.address" custom-class="mot-input-text" placeholder="请输入地址" type="text" />
      </mot-form-item>
      <mot-cell>
        <mot-button type="primary" size="small" custom-style="margin-right: 10px" @click="submit">
          提交
        </mot-button>
    ...
    <mot-form label-position="top" star-position="right">
      ...
      <mot-form-item label-position="left" label="备注">
        <mot-textarea placeholder="请输入备注" type="text" />
      </mot-form-item>
    </mot-form>
  </div>
