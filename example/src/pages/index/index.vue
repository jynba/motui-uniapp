...
<template>
  <view class="index t-bg-2">
    ...
    <view class="index-components t-bg">
        ...
        <template v-if="_nav.show">
          ...
          <div class="info">
            <template v-for="_package in reorder(_nav.packages as any)" :key="_package.name">
              <view v-if="_package.show && _package.uni && _package.exportEmpty !== false" class="info t-bg-2 t-text-color">
                <text class="link" @click="navigateTo(_package.name, _nav.enName)">
                  {{ _package.name }}
                  &nbsp;&nbsp;
                  {{ _package.cName }}
                </text>
                <mot-icon size="14px" color="#979797" name="right" />
              </view>
            </template>
          </div>
        </template>
